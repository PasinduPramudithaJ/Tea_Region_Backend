...
resnet18_model.load_state_dict(torch.load(os.path.join(MODELS_DIR, "tea_4_region_model_restnet18.pth"), map_location=device))
resnet18_model.to(device).eval()
models_dict["tea_4_region_model_restnet18"] = resnet18_model

# --- ResNet4 ---
resnet4_model = ResNet4(NUM_CLASSES)
resnet4_model.load_state_dict(torch.load(os.path.join(MODELS_DIR, "tea_4_region_model_restnet4.pth"), map_location=device))
...
def health_check():
    ...
    You can expand this to check models, DB, etc.
    """
    try:
        # Example: simulate model check or DB ping
        # model_loaded = True
...
# ----------------------------------------------------------
model = joblib.load("models/best_region_model.pkl")
encoder = joblib.load("models/region_encoder_cv.pkl")
